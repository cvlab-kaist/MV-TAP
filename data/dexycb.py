...
class DexYCB(torch.utils.data.Dataset):
    def __init__(
        ...
        for seq_path in seq_paths:
            ...
            num_total_views = len(all_view_paths)
    ...
    def __getitem__(self, index):
        ...
        npz_filename = 'tracks_3d.npz'
        tracks_key = 'tracks_2d'
        visibility_key = 'tracks_2d_visibilities'
        ...
        for view_path in all_view_paths:
            cam_path = os.path.join(view_path, 'intrinsics_extrinsics.npz')
            cam_data = np.load(cam_path)
            intrinsics.append(cam_data['intrinsics'][:3, :3])
            extrinsics.append(cam_data['extrinsics'])
